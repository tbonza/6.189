...
class Board(object):
    ...
    def __init__(self, win, width, height):
        ...
        for x in range(0, self.width):
            ...
            for y in range(0, self.height):
                ...
                # Get the block
                self.block_list[(x, y)] = Block(Point(x,y), 'NavyBlue')
    ...
    def seed(self, block_coords):
        ...
        for point in block_coords:
            ...
            #separate point to self.x, self.y
            self.x = point[0]
            self.y = point[1]
    
            # Set the squares to LIVE
            self.block_list[(self.x, self.y)].set_live(self.canvas)
    ...
    def get_block_neighbors(self, block):
        ...
        for x in range(self.x - 1, self.x + 2):
            ...
            #Edge conditions: x can't be negative, self.x >= 0 <= 10
            if 0 <= x and 0 <= self.x <= 10:
                for y in range(self.y - 1, self.y + 2):
                    
                    # Do the checked coordinates match the 
                    # list coordinates?
                    if not (x, y) == (self.x, self.y) and 0 <= y <= 10:
                        self.neighbors_list.append(self.block_list[(x, y)])
    ...
    def simulate(self):
        ...
        # Find neighbors
        neighbors = self.block_list.values()
        
        live_neighbors = 0
        
        for neighbor in neighbors:
            
            # Enter block neighbors into new list
            neighbors_list = self.get_block_neighbors(neighbor)
            
            # Check to see if neighbor is alive
            for x in range(len(neighbors_list)):
                if neighbors_list[x].is_live() == True:
                    
                    # count of live neighbors
                    live_neighbors += 1
                    
            # Set new neighbor status based on new rules
            if live_neighbors < 2:
                neighbor.new_status = 'dead'
            elif live_neighbors == 2 and neighbor.is_live():
                neighbor.new_status = 'live'
            elif live_neighbors == 3:
                neighbor.new_status = 'live'
            else:
                neighbor.new_status = 'dead'
                
            # Reset live_neighbors to 0
            live_neighbors = 0
            
        # Reset the status for all blocks after they've
        # been checked
        for neighbor in neighbors:
            neighbor.reset_status(self.canvas)
