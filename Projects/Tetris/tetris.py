...
class Tetris():
    ...
    def create_new_shape(self):
        ...
        # choose a random shape and return it
        new_shape = randint(0, len(self.SHAPES) - 1)
        shape = self.SHAPES[new_shape](Point(int(self.BOARD_WIDTH/2), 0))
        return shape

        # use Tetris.__init__ to display shape
    ...
    def do_move(self, direction):
        ...
        if self.current_shape.can_move(self.board, self.DIRECTION[direction][0],
            ...
            return True
        elif not self.current_shape.can_move(self.board, self.DIRECTION[direction][0],
                                             self.DIRECTION[direction][1])\
                                             and direction == 'Down':
            return True
        else:
